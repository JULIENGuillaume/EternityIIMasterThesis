...
namespace etm {
	...
	protected:
		void generateFuturePopWithCrossover() override;
	...
	protected:
		uint32_t findPieceAgreement(const std::vector<std::pair<uint32_t, uint32_t>> &child,
		                            ...
		                            uint32_t pos);
		uint32_t findBestBuddy(const std::vector<std::pair<uint32_t, uint32_t>> &child,
		                       const std::vector<std::pair<uint32_t, uint32_t>> &parent1,
		                       const std::vector<std::pair<uint32_t, uint32_t>> &parent2,
		                       std::vector<uint32_t> const &piecesAvailable,
		                       uint32_t pos);
		uint32_t findBestFitAvailable(const std::vector<std::pair<uint32_t, uint32_t>> &child,
		                              const std::vector<std::pair<uint32_t, uint32_t>> &parent1,
		                              const std::vector<std::pair<uint32_t, uint32_t>> &parent2,
		                              std::vector<uint32_t> const &piecesAvailable,
		                              uint32_t pos);
